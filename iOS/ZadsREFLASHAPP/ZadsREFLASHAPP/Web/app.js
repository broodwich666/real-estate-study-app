...
function nav(view){$$('.view').forEach(v=>v.classList.toggle('active',v.id===view));$$('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));$('#pageTitle').textContent={dashboard:'Dashboard',brain:'Brain',notes:'Personal Notes',work:'Work Mode',vault:'Vault',settings:'Settings'}[view];$('#eyebrow').textContent=view==='work'?'REAL ESTATE OPERATIONS':view.toUpperCase();if(view==='vault')renderVault();if(view==='work')renderWork('agenda');if(view==='notes')renderNotesList()}
...
function setPopup(on){state.popup=on;clearInterval(popupTimer);$('#popupBtn').textContent=`Pop-up cards: ${on?'On':'Off'}`;if(on){popupTimer=setInterval(()=>{const c=shuffled(chapters.flatMap(x=>x.cards))[0];startStudy([c],'Pop-up Review')},state.interval*60000)}save()}
