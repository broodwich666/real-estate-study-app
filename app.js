...
function nav(view){$$('.view').forEach(v=>v.classList.toggle('active',v.id===view));$$('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));$('#pageTitle').textContent={dashboard:'Dashboard',brain:'Brain',notes:'Personal Notes',work:'Work Mode',vault:'Vault',inventory:'NY Inventory',mapper:'Property Map',settings:'Settings'}[view];$('#eyebrow').textContent=view==='work'?'REAL ESTATE OPERATIONS':view.toUpperCase();if(view==='inventory'&&window.renderInventory)window.renderInventory();if(view==='vault')renderVault();if(view==='work')renderWork('agenda');if(view==='notes')renderNotesList()}
